...
#         max_length=64, unique=True, blank=True, null=True, help_text="Subdomain prefix for this vendor (e.g., carbonilab).")

#     # ACCESS & SUBSCRIPTION
#     is_active = models.BooleanField(default=False, help_text="Set to False if subscription lapses or vendor is disabled.")
#     activation_email_sent = models.BooleanField(default=False, help_text="Whether activation email has been sent to tenant admin")
    
#     plan_type = models.CharField(max_length=50, choices=PLAN_CHOICES, default='BASIC', help_text="Plan type for this vendor.",)
...
#     def __init__(self, *args, **kwargs):
#         super().__init__(*args, **kwargs)
#         # Store the original state to check for changes later
#         self.__original_is_active = self.is_active
...
#             and not self.activation_email_sent
#         ):
#             send_vendor_activation_email_task.delay(self.internal_id)
...
#                 "Domain cannot be modified after vendor activation."
#             )
...
# # Celery operation -- Async
# @shared_task(bind=True, autoretry_for=(Exception,), retry_backoff=30, retry_kwargs={'max_retries': 3})
# def send_vendor_activation_email_task(self, vendor_id):
#     """
#     Idempotent async task to send vendor activation email.
#     """
#     with transaction.atomic():
#         vendor = Vendor.objects.select_for_update().get(id=vendor_id)

#         if vendor.activation_email_sent:
#             return "Activation email already sent"

#         send_vendor_activation_email(vendor)

#         vendor.activation_email_sent = True
#         vendor.save(update_fields=["activation_email_sent"])
